...
export function ControlledForm() {
    ...
    function handleSubmit(event) {
        ...
        const uplaodedFile = fileRef.current?.file?.[0] || null;

        const submittedData = {
            ...formData,
            uplaodedFileName : uplaodedFile ? uplaodedFile.name : "No file selected"
        };
    ...
    return(
        <section>
            ...
            <form onSubmit={handleSubmit}>
                <div>
                    <input type="text" name="username" id="username" value={formData.username} onChange={handleChange} placeholder="Enter User Name"/> <br />
                    <textarea name="bio" id="bio" value={formData.bio} onChange={handleChange} placeholder="Write a short bio"></textarea> <br />
                    ...
                    <br />
                        <label htmlFor="agree">
                        <input type="checkbox" name="agree" id="agree" checked={formData.agree === "Female"} onChange={handleChange}/>
                        Accept terms
                        </label>
                        <br />
            ...
            <p>Selected file: { fileRef.current?.files?.[0]?.name || "No file slected" }</p>
        </section>
    );
}